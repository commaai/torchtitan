...
def compute_worldmodel_losses(
    ...
    *,
    ...
    if "sample" in outputs:
        ...
        diffusion_loss = (mse * flat_mask).sum(dim=1) / flat_mask.sum(dim=1).clamp_min(
            1.0
        )
    ...
    if "plan" in outputs and "plan" in targets:
        ...
        plan_loss_values, plan_err, plan_mask = laplacian_density_loss(
            plan_target.float(),
            plan_pred.float(),
        )
        plan_loss = plan_loss_values.flatten(1).mean(dim=1)
        flat_plan_mask = plan_mask.flatten(1).float()
        plan_mse = (plan_err.square().flatten(1) * flat_plan_mask).sum(
            dim=1
        ) / flat_plan_mask.sum(dim=1)
