...
STRUCTURED_LOG_DIR = os.getenv(
    ...
    "/tmp/torchtitan_train/wanworldmodel_torchpackage_checkpoint",
)
_WAN_TORCH_PACKAGE_MODULE = (
    "torchtitan.experiments.wanworldmodel.torchpackage_checkpoint"
)
WAN_TORCH_PACKAGE_RECIPE = f"{_WAN_TORCH_PACKAGE_MODULE}:WanTorchPackageRecipe"
WAN_TRAINING_TORCH_PACKAGE_RECIPE = (
    f"{_WAN_TORCH_PACKAGE_MODULE}:WanTrainingTorchPackageRecipe"
)
...
TORCH_EXPORT_INTERN_MODULES = [
    ...
    "torchtitan.experiments.wanworldmodel.model_for_inference",
    "torchtitan.experiments.wanworldmodel.model",
...
def validate_model_config(state: Any) -> WanModel.Config:
    ...
    if not isinstance(state, WanModel.Config):
        raise TypeError(
            "Wan worldmodel torch package state must be WanModel.Config or "
            f"WanTorchPackageConfig, got {type(state).__name__}."
        )
    return state
...
def build_package(
    *,
    ...
    with sl.log_trace_span(f"wan_package_convert_{weight_format}"):
    ...
    with sl.log_trace_span("wan_package_model_io"):
    ...
    with sl.log_trace_span("wan_package_build"):
        ...
        del package_buffer
    sl.log_trace_scalar({"wan_package.package_bytes": len(package)})
    return package


@dataclass(slots=True)
class WanTorchPackageRecipe:
    weight_format: WeightFormat = DEFAULT_WEIGHT_FORMAT
...
class WanTrainingTorchPackageRecipe(WanTorchPackageRecipe):
...
def export_torch_package(
    ...
    *,
    ...
    if model_flavor is None:
        ...
        from torchtitan.experiments.wanworldmodel.model_config import model_registry

        package_config = model_registry(model_flavor).model
    try:
        export_recipe_torch_package(
            recipe=WanTorchPackageRecipe(weight_format=weight_format),
...
class WanTorchPackageCheckpointManager(TorchPackageCheckpointManager):
    """Wan worldmodel checkpoint manager configured with the Wan recipe."""

    @dataclass(kw_only=True, slots=True)
    class Config(TorchPackageCheckpointManager.Config):
        torch_package_recipe: str = WAN_TRAINING_TORCH_PACKAGE_RECIPE
...
def main() -> None:
    ...
    sl.init_structured_logger(
        source="wanworldmodel_torchpackage_checkpoint",
        output_dir=STRUCTURED_LOG_DIR,
    )
    with sl.log_trace_span("wan_package_total"):
