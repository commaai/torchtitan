...
class TestCheckpointManager(unittest.TestCase):
    ...
    def test_custom_checkpoint_id_format_finds_latest(self):
        ckpt_folder = os.path.join(self.test_folder, "custom")
        os.makedirs(ckpt_folder, exist_ok=True)
        for step in (2, 5):
            d = os.path.join(ckpt_folder, f"epoch_{step}")
            os.makedirs(d, exist_ok=True)
            open(os.path.join(d, ".metadata"), "w").close()
...
class TestConfigPostInit(unittest.TestCase):
    ...
    def test_path_normalization(self):
        """Test that paths are stripped and must be absolute or URL-like."""
