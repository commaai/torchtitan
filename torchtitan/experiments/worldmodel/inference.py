...
from torchtitan.experiments.worldmodel.model import TensorOrMask, WorldModel, _dense_mask, _mask_fn
...
class WorldModelForInference(WorldModel):
    ...
    def setup_inference_attention_attrs(self, device: torch.device, num_conditioning_frames: int) -> None:
        ...
        elif self.config.transformer.attention_impl in {"SDPA", "NAIVE"}:
            prefill_mask = None
            if cut_off:
    ...
    def _has_compatible_caches(
        ...
        *,
        ...
        return all(
            block.attn.kv_cache is not None and block.attn.kv_cache.k_cache.device == device
            for block in self.blocks
        )
    ...
    def generate(
        ...
        **scheduler_kwargs: Any,
        ...
        assert (
            self.config.transformer.attention_mask != "NONE" or num_conditioning_frames == 0
        ...
        if steps <= 0:
            ...
            output_latents = self.unscale_latents(latents[:, num_conditioning_frames - 1 :])
        ...
        assert frames >= num_conditioning_frames, f"{frames} < {num_conditioning_frames}"
